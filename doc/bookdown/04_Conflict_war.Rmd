...
+ `prob_split`: This defines the probability that, given the minimum number of individuals is reached, the group will actually split and relocate. It is also culture-dependent.
...
We then combine this first friction surface with the ressource raster to compute another friction map representing weighted average probability, where the distance to origin accounts for 70% of the final probability and available resources 30%.
...
- `max`() returns the maximal value of a raster, to normalise value as probabilites (it's implemented by `terra::minmax()` function)
...
This is wrapped within the function `run_simulation`, that will be described in detail in the [next chapter](#main-simu). Here is a simplified version of the section of the code that handles these migration events:
...
During each time step of the simulation, each settlement has the opportunity to fight against another settlement. This again, will depend on multiple factor such as the culture of the settlements and their population sizes. Here is a breakdown of how war is simulated according to the provided code snippet:

1. **Identification of Aggressors**:
    - In the archaeoriddle simulation, only farmers have a aggressive behavior and engage in conflict. Thus during a first step, farmer settlements with a population size greater than 50 are identified as potential aggressors (`potentialfighters`). From this subset of the farmer's settlements,10% of them will actually try to engage in warfare.

5. **Selecting the Target for Attack**:
    - Settlements within a radius from the agressor and with a different culture  are identified as potential victims. The radius is defined by theparameters `bufferatack`.
    Then:
...
The model with generate losses on both sides, but on average less for the biggest population, while still allowing cases where the smaller settlement wins the fight and generates more damage than it receives. These differences are smoothed when the size of the two settlements become similar.
...
- `whotouch` :  is a central element of the conflict, and it returns the list of sites that are within the reach of the settlements. A couple of adjustments have been manually done for the original challenge: only farmers attack, and they do so only if they have at least 50 individuals. Among the settlements that have the potential to attack another, they will do so with a probability of `probfight`. This has been set to 0.1 in the original challenge in order to ensure that settlements do not fight everyone all the time, which was leading to way too much wars.
...
Here are defined lot of the assumptions that will be tested by the research questions proposed in the original challenge described in the Chapter&nbsp;\@ref(original-challenge).
...
Population will then grow, for years and years. This implies multiple things: population needs resources and a way to grow with regard to these resources and their own technologies. We initialised the resource layer in the previous chapter 
in [the  section about resources](#env). In the following sections we will use this ressource raster to initialise site size and define how resources are used by the different cultures.
