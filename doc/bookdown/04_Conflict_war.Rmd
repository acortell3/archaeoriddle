...
$$
\text{attractivity} = \exp\left(\text{resources} \times \text{space} \times \log(\text{distance})\right) / \sum \exp\left(\text{resources} \times \text{space} \times \log(\text{distance})\right)
$$
...
7. **Recording War Casualties and War Events**:
    - Casualties are recorded, and a `warpoints` function visualizes or logs the event, using the calculated size of the war and any additional parameters (like a fixed size increase and plotting options).

8. **Adjusting Populations**:
    - Population sizes are adjusted according to the battle outcomes. This adjustment simulates the "effectively killing people in the population." Ideally, this adjustment should consider age demographics for more realism.
...
poitentialfighters <- which(sites$culture=="F" & Nts[i,]>50)
