...
An important part of the simulation is to generate a landscape and an environment. Doing so often rely on
...
This autocorelate time series can be used to adjust temporal change in climate (rain, temperature, etc....). But we also need a way to generate elevation and slope. We can use similar methods to generate 2D autocorellation. We use here an implementation of what is known as [perlin noise](https://en.wikipedia.org/wiki/Perlin_noise) found on [this stackoverflow.com post](https://stackoverflow.com/questions/15387328/realistic-simulated-elevation-data-in-r-perlin-noise)
...
Different parameters for the perlin noise will give us a higher a lower definition for our future DEM raster ; here we choose a tradeoff between an environment detailled enough for on a relatively wide area but small enough to be handled easily in memory.
...
height.ras=rast(height,extent=c(-3.5,.5,-.5,3.5),crs="+proj=latlon")
...
plot_3d(hm_lamb,height_map, zscale = 10, fov = 0, theta = 20, zoom = .5, phi = 45, windowsize = c(1000, 800),water = TRUE, waterdepth = 0, wateralpha = 1, watercolor = "lightblue", waterlinecolor = "lightblue", waterlinealpha = .7,baseshape="hex")
...
Given this new landscape we can now
...
```{r,eval=F}
coastline=st_as_sf(as.polygons(height.ras>mean(height.ras[])))[2,]  ##get only the non submerged actual land
plot(coastline,col=NA,bgc=adjustcolor("cyan",.1),add=T)
st_write(dsn="coastline2.shp",coastline)
writeRaster(filename="east_narnia4x.tif",height.ras,overwrite=T)
```

## Network, Site Size And Climatte -- DEPRECATED
...
render_points(extent = raster::extent(raster::raster(height.ras)), lat = 2.13828, long = -1.685547, altitude =extract(height.ras,sites)[,1]+20, zscale=20,size=1,col=categorical_pal(max(allcomu))[allcomu],clear_previous = TRUE)
