...
This generates autocorrelated time series that can be used to define temporal change in climate (rain, temperature, etc.). But we have not used such changes for the Archaeoriddle challenge. Coloured noise remains a very straightforward way to simulated factor and could be easily added in the simulations.

## Slope and Elevation

An important aspect of the simulation is a way to generate elevation and slope. To generate 2D autocorrelation we used a method known as [perlin noise](https://en.wikipedia.org/wiki/Perlin_noise) ; using the implementation found on [this stackoverflow.com post](https://stackoverflow.com/questions/15387328/realistic-simulated-elevation-data-in-r-perlin-noise).
...
We can then visualise the result as a 2D raster. We set up the total extent of the raster to define exactly where it is on our GIS using a latlon projection.

We also define the mean height as the water level : All the cells below are then considered under water.
...
The resources will be located at given hotspots with an area around in which they decay according to a logistic function. We provide a very basic function `logisticdecay` which allows you to define multiple sources with different decay arrays around them.
...
These resources span along all the map but we want to restrict them to places where people will be able to settle.
Given this new landscape we can now generate sites in specific locations. We first need to define _where_ the sites _can_ appear (not in water nor too high in the mountains) ; so we first create a raster 'viable' which will give a binary map where groups of human can live or not.
...
We now have also have a polygon `viable` representing the space where sites can be created.
