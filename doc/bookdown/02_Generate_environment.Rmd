...
If we change k (that dictates the total resolution of the DEM), the size of the raster could grow exponentially and quickly become hard to manage.
...
We also define the mean high as the water level, below which everything is considered under water.
...
We have generated zones of higher resources/fitness that attract settlements and support higher carrying capacities _ie_ population size.
The resources are located at given hotspots with an area around in which they decay
with a logistic function. We provide a very basic function `logisticdecay` which allows to define multiple sources with different decay arrays around them.
...
When considering the selection of hotspots, one may choose to retain the one utilized for the _archaeoriddle_ but to adjust these resources according to your own topography one can use `locator()` to find the coordinates of the hotspot by clicking on the map. An example of that is given in [this  section of the chapter](#manual-selection)

These resources span along all the map but we want to restrict them to places where people will be able to settle.
Given this new landscape we can now generate sites in specific locations. We first need to define _where_ the sites _can_ appear (not in water nor too high in the mountains), so we first create a raster 'viable' which will give a binary map where groups of humans can leave or not.
...
And then we add the resources raster, which gives us a final `ress` raster as below:
