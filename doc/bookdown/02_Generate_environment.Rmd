...
Given this new landscape we can now generate sites in specific locations. We first need to define _where_ the sites _can_ appear (not in water nor too high in the mountains)
...
We now have a polygon `viable` representing the space where sites can be create.
...
We can now use simply and directly use `spatSample` from package `terra` to randomly select `n` points  (`r n` for this simulation) on this viable polygon.
...
In the context of [the archaeoriddle challenge](https://theia.arch.cam.ac.uk/archaeoriddle/), files that describe this newly generated environment and the sites sampled on top of it needs to be produced and given to the participant. This can be done as:
