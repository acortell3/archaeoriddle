...
To simulate the interaction between two groups, we need an environment where they can interact. This chapter covers how to generate a landscape and an environment for your simulated groups.
An essential part of the simulation is to generate a realistic landscape and environment. Environmental variables are often auto-correlated. For example, the temperature recorded at a given time is similar to that recorded just before or after that particular moment. Similarly, topographic variables such as slope or elevation tend to be similar at a shorter rather than larger distances.

## Autocorrelated noise generation

One way to generate such an auto-correlated variable is to use [coloured noise](https://en.wikipedia.org/wiki/Colors_of_noise). This uses a specific distribution to define the relation between frequencies found in your signal with the power of these frequencies. In [pink noise](https://en.wikipedia.org/wiki/Pink_noise) (or $\frac{1}{f}$-noise) for example, the power of each frequencis will be inversely proportional to the frequencies (_ie_ you will have, low frequencies with high power, high frequencies with low ones).  We defined an `environment` function, which in turns uses `TK95` to generate a power law noise.
...
This generates autocorrelated time series that can be used to define temporal change in climate (rain, temperature, etc.). But we have not used such changes for the Archaeoriddle challenge. Coloured noise remains a very straightforward way that can be easily added in the simulations.

## Slope and Elevation

An important aspect of the simulation is to generate realistic elevation and slope values. To generate a 2D autocorrelated variable we used a method known as [perlin noise](https://en.wikipedia.org/wiki/Perlin_noise) ; using the implementation found on [this stackoverflow.com post](https://stackoverflow.com/questions/15387328/realistic-simulated-elevation-data-in-r-perlin-noise).
...
If we change k (that dictates the total resolution of the DEM), the size of the raster grows explonentially and would quickly become hard to manage.
...
To enhance realism, slight adjustments were made to the extremes to create water bodies and mountains more closely resembling those found in an real landscape.
...
We can then visualise the result as a 2D raster. We set up the total extent of the raster to define precisely where it is on our GIS using geographic coordinates.

We also define the mean height as the water level: All the cells below are considered water bodies.
...
This has been put in the function `plotMap`
...
The resources will be located at given hotspots with an area around which they decay following a sigmoid function. We provide a very basic function `logisticdecay` which allows you to define multiple sources with different decay arrays around them.
...
When considering the selection of hotspots, you may choose to retain the one utilized for the _archaeoriddle_ but to adjust these resources center to your own topography ;  one can use `locator()` to find the coordinates of the hotspot by clicking on the map. An example of that is given in [this  section of the chapter](#manual-selection)

These resources span along all the map, but we want to restrict them to places where people can settle.
Given this new landscape, we can now generate sites in specific locations. We first need to define _where_ the sites _can_ appear (not in water nor too high in the mountains), so we first create a raster called 'viable', a binary map defining where groups can or cannot live.
...
We now also have a polygon called `viable` representing the space where sites can be created.
This space is represented by the green area on the map below.

```{r,viablearea}
plot(viable,col=adjustcolor("chartreuse",.8))
```

We then loop over every center manually defined previously and apply the logistic decay within the `viable` raster.
...
And then we add all the resource rasters, which gives us a final `ress` raster as below:
...
Below is some useful code to manually select the resource origins on the map (by clicking on the screen) and explore different decays around them
