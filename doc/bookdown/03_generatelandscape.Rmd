...
height.ras=rast("east_narnia.tif")
...
```{r,eval=F}
coastline=st_as_sf(as.polygons(height.ras>0))[2,]  ##get only the non submerged actual land
plot(coastline,col=NA,bgc=adjustcolor("cyan",.1),add=T)
st_write(dsn="coastline.shp",coastline)
writeRaster(filename="east_narnia.tif",height.ras,overwrite=T)
```
