...
plot((height.ras)^1.9,col=terrain.colors(50),legend=F,reset=F)
...
Visualise sites on the 3D plots

```{r 3Dnetwork,cache=F,eval=T}
height_map |>
  sphere_shade(texture = "imhof1",zscale=5) |>
  add_shadow(ray_shade(height_map, zscale = 19), 0.8) |>
  add_shadow(ambient_shade(height_map), 0,5) |>
  add_shadow(lamb_shade(height_map), 0) |>
  plot_3d(height_map, zscale = 20, fov = 0, theta = 135, zoom = 1, phi = 65, windowsize = c(1000, 800),water = TRUE, waterdepth = mean(height), wateralpha = 1, watercolor = "lightblue", waterlinecolor = "lightblue", waterlinealpha = .7)
...
render_points(extent = raster::extent(raster::raster(height.ras)),
             ...
             zscale=20,size=2,col="red",clear_previous = TRUE)
       
render_highquality(point_radius=2,samples=256, clear = TRUE,light=TRUE)
