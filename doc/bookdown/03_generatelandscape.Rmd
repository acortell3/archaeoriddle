...
rescal `m` as positive integer
```{r,cache=F}
height=(m[,])*20
```

Tweak a bit the perlin noise to have more difference between low area and high are
...
height_map = rayshader::raster_to_matrix(raster(height)) 


height_map |>
  sphere_shade(texture = "imhof1",zscale=5) |>
  add_shadow(ray_shade(height_map, zscale = 19), 0.8) |>
  add_shadow(ambient_shade(height_map), 0,5) |>
  add_shadow(lamb_shade(height_map), 0) |>
  plot_3d(height_map, zscale = 10, fov = 0, theta = 135, zoom = 1, phi = 65, windowsize = c(1000, 800),water = TRUE, waterdepth = mean(height), wateralpha = 1, watercolor = "lightblue", waterlinecolor = "lightblue", waterlinealpha = .7,baseshape="hex")

Sys.sleep(0.2)
render_highquality( samples=256, clear = TRUE,parallel=TRUE,light=TRUE)
...
```{r,cache=F}
height.ras=rast(height)
plot(height.ras,col=terrain.colors(50),legend=F,reset=F)
height.ras=rast(height)
height.ras[height.ras>mean(height.ras[])]=NA
plot(height.ras,col="lightblue",add=T,legend=F)
height.ras=rast(height)
maxh=max(height.ras[],na.rm=T)
height.ras[height.ras<mean(height.ras[])]=NA
height.ras[height.ras<(maxh*.7)]=1
height.ras[height.ras>(maxh*.7)]=200
height.ras[is.na(height.ras)]=-1
height.poly=as.polygons(height.ras)
height.poly=as.polygons(height.ras)
viable=height.poly[2,]
n=10
size=rexp(n)
sites=spatSample(viable,n)
plot(sites,cex=2,pch=21,add=T,bg=rainbow(10,alpha=.6))

```

Add a couple of sites with exponentially distributed size and make a network out of it.  
