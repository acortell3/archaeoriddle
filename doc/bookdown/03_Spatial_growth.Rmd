...
In the context of [the archaeoriddle challenge](https://theia.arch.cam.ac.uk/archaeoriddle/), the files that describe this newly generated environment (the DEM and the coastline) as well as the resources raster,  needed to be given to each participant. This can be done as follows:
...
Each site is then assigned as either belonging to the *Rabbitskinners* (Hunter-gatherers) or the *Poppychewers* (Farmers).

To generate a more realistic distribution, all sites are initially designated as hunter-gatherer sites, except those in the southwest portion of the landscape. Please note that this decision was based on the specific narrative of the initial archaeoriddle's challenge. If needed, you may want to manually select the site culture for any specific location.
...
Each site will then be assigned a carrying capacity (how many people can leave on the site), which will be probabilistically defined using the resource raster and the cultural affiliation of the site.

```{r computeK}

Kbase <- c("HG"=45,"F"=120) #difference in base-K for the two cultures

# The base Ks  are then modulated given the resource raster
sites$Ks <- initKs(Kbase, sites, ress, sizeexp="F")
```

Using these carrying capacities, we then set the initial number of people, and the population structure for each site:
...
This is a core aspect of the model, and defines several parameters to control how populations grow and reach a maximum size $\sim K$ ($K$ is the carrying capacity). It is based on three functions: The first function, `Gpd()`, is the core of the protocol. It receives a population `matrix` or `data.frame` with a specific number of individuals (`nrow`), their age (`df[,1]`) and their sex (`df[,2]`) and produces another object with the same format but with a different population based on the current one. Introduced in a loop, this function produces a time series of population, which ensures the Markov property of the stochastic process $p(x_n+1|x_n)$.

However, this function also relies on two other functions: `death()` and `K_lim()`. `death()` is the process of people dying each year. It is based on an age-structured probability matrix, where infants and old people have higher probabilities of death. The matrix is provided, but users can set their own version. `K_lim()` limits the population when it reaches $K$. Users should specify the appropriate value for $K$ according to the size of the site, its type, the group economy, etc. Also, users can change the probability that exceeding people dies with parameter `prob`. The default value we used is 0.8.




These base functions will then be used in higher-level functions to update the population size given the size and ages defined and described in the previous [section](#initsize). 

A simple function  `Pop_stoch` implements a simple stochastic growth that uses these functions without the migrations and war mechanisms described in this chapter. 


We can then use it to check and see the type of growth that is generated given the default parameters (prob defines the probability of people dying when the population has reached carrying capacity)
...
You can also use this [shiny app online](https://acortell3.shinyapps.io/archaeo_record_model/) to explore how these parameters interact to generate different growth curves.
