...
However, in most cases, this information will not be known to the field researcher, and in this situation, these variables can be modelled to different extents of complexity. While, for example, in the case of the most simple simulation, the field archaeologist could ignore deposition rates. In this case, and because we are only considering 14C dates, then the quantity of the sample becomes broadly proportional to the number of people present in a site, and thus it could be modelled like this.
...
However, this function also relies on two the other functions: `death()` and `K_lim()`.
...
$$A(t)= Pop(t)\times O(t)\times F(t) \times G(t),\\ \text{with}\ t = 1,2,...,T_{death} \tag{2}$$
Where:
...
* And $C(t)$ is the quantity of kilocalories, extracted from meat, consumed by person for each time $t$, and it is defined as $C(t)=B(t) \times M(t) \tag{3}$, where 
...
This protocol is designed to respond for the fact that, despite a bone is deposited in a time $t$, it can occupy the stratigraphic space of many subsquent years until it is fully buried. The years it takes to be fully buried depends on the thickness of the bone and the natural deposition rates $r$.

Since the thickest bones are usually the minority within a sample, whereas the majority is usually conformed by smaller bones, we have considered that bone presence diminishes exponentially as it departs from $t_0$, being $t_0$ the year when it was deposited. Thus, this process can be modelled as follows:
...
* $t_u$ is each time $t$, $t \geq t_l$, when the sample is being buried according to the parameter $r$. Thus, at $t_{u_{max}}$, the sample is completely buried.
...
3. D-protocol. Knowing how many samples are generate each year, using Rec_c, we distribute them along the record.
