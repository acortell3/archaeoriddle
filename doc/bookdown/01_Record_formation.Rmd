...
* **Waste produced** ($W/m^2$ or just $W$) **(outcome variable)**: Archaeological record present per $m^2$. Since we are considering C14 dates, then this is a proportion of the organic material that could be found in a site. We have considered these a discrete variable (number of potential samples) with range $[0,\propto K]$. *Defined in code as* `Wp`.
* **Population (group size)** ($Pop$): Discrete variable with range $[0,\sim K]$. Population group size is key to determine the anthropogenic deposition rates (which in turn determine $W$) but it can also be used as a proxy to determine $W$ in case those are not accounted for since, in that case, $W \propto Pop$. Although this variable can, like the rest, be set as a constant by the user, this is unadvised, as demographic processes are rarely constant. In any case, the user does have to provide a value for the carrying capacity ($K$) as this will limit population growth. The user must infer carrying capacity (maximum possible number of people within a site) based on prior expert knowledge. *Defined in code as* `Pop`.
* **Anthropogenic deposition rates** ($Adr$): This is a continuous variable, with range $[0,\infty]$, measured in *mm/year*. Ultimately, this should define the amount of waste produced in a site, and we have considered its possible conditioning on two other variables: (1) $Pop$ and (2) $Site function$ or $Sf$. The user can decide to condition $Adr$ on $Pop$, on $Sf$, on both or on none, by providing directly the value. *Defined in code as* `Adr`.  <font size ="0.5"> *The upper range must be reassessed based on literature* </font>.
* **ONatural deposition rates** ($Ndr$): This is a continuous variable, with range $[0,\infty]$, measured in *mm/year*. Deposition rates from natural causes can heavily condition the formation process of the archaeological record. In turn, the type of site (whether it is an open-air site, a cave or a rockshelter) can severely affect them. As in all the other cases, the user can chose to set them, if known, to model them or to disable them. *Defined in code as* `Ndr`. <font size ="0.5"> *The upper range must be reassessed based on literature* </font>.
...
In order to fully understand the process, first we will show the protocoles and functions that we use to generate each variable.
...
### Anthropogenic deposition rates ($Adr$) Adr-protocol

This generates the amount of human waste produced per year. It can be used either directly from the previous Pop-protocol, only taking into account the site function (which unavoidably yields a fixed rate for all the period considered) or, the most advisable, as a combination of the previous processes. It contains one function `A_dep_r()`, which includes one parameter `S_f`, defining the function of the site. If `S_f = NULL`, then the site function has no effect. Alternatively, the parameter `Pop` can be introduced manually, or retrieved from the previuous protocol. If the parameter `Pop = NULL`, population has no effect on deposition rates. If both `Pop` and `S_f` are active, they both configure the final `Adr` outcome. However, if both are `NULL`, then the function will throw an error. In that case, the user will have to introduce manually the anthropogenic deposition rates for the computation of $W$ in subsequent processes.

```{r}

## Function 4. Simulation of anthropogenic deposition rates
#' Returns the total amount of waste produced in the site. An integer.
#' @title A_dep_r
...
### Natural deposition rates ($Ndr$) protocol
Natural deposition rates usually have an effect on the archaeological waste product, where the density of $W$ is usually inversely proportional to the speed of the natural deposition rates. This protocol consists of one function `N_dep_r()` where the argument `S_t` (site type) can be enabled or disabled. If disabled, natural deposition rates are only considered through the parameter `speed`. When enabled it conditions the deposition rates on the type of site (open-air, cave or rockshelter). We are aware that there are many causes determining natural deposition rates. However, for the sake of simplicity, we have decided to include only one extra layer, the parameter `speed`, accounting for the speed of such rates, disregarding the causes which might provoke that speed.

```{r}
## Function 5. Simulation of deposition deposition rates
#' Returns the rate at which sediment is deposited. Double.
#' @title N_dep_r
...
#' @param speed: This is the speed at which the natural sediment deposits on the 
...
W_prod <- function(x,Adr = NULL,Ndr = NULL, area){
...
Once this is done, we can implement the protocol one, where we compute the population during the desired time span. For this first simulation, we have decided to leave all values as the default values, this meaning a medium $Ndr$, for an open-air and habitat site.
...
In this case, we evaluate different values for the simulation, with different fertile ages for men and women, a slower fertility rate, and considering a cave with slow deposition rates, being a logistic camp with two months of total occupation per year.
...
for (i in 1:ts){
  Pop <- Gpd(Pop, W_fer_age = c(15,35), M_fer_age = c(15,30))
  a_w <- A_dep_r(Pop, S_f = "l", occ = 2)
  ndp <- N_dep_r(S_t = "c", speed = "s")
...
Interestingly, comparing this second simulation to the first one, we observe that the correlation between population and waste produced is not as strong as it was before, but also that less waste is produced in general for this type of occupation, being the case that archaeological waste is actually non-existen even for periods where the site was being occupied.
