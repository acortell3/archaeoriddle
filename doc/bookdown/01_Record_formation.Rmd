...
>>>>>>> c197ba00bf2a8537e2e0068ae942ac4db751d291
...
$$A(t)= P(t)\times O(t)\times F(t) \times G(t) \tag{1}$$
Where:

* $P(t)$ is a discrete variable representing the number of living people at a time $t$.
...
$$W_{b_{(t_u,t_l)}}=W(t=t_l)\times(1-e^{-\lambda(t_u-t_l)}) \tag{5}$$
...
$$\frac{W_b(t_{u_{max}}=1,t_l)}{W(t_l=1)} \tag{7}$$
...
$$1-e^{-\lambda t_{u_{max}}}=P_b$$

and thus

$$\lambda=\frac{-log(1-P_b)}{t_{u_{max}}} \tag{9}$$

Where $P_b$ is the proportion of buried samples in $t_{u_{max}}$
...
With which we can obtain, among other things, the distribution of the samples for each specific date along the depth of the sit, and its comparison to the total population. For better comprehension, we have decided not to include all of the dates in the plot below, but only some of them as an example.
