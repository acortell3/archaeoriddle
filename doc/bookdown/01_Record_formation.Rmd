...
This generates the amount of human waste produced per year. It can be used either directly from the previous Pop-protocol, only taking into account the site function (which unavoidably normalises the results) or, the most advisable, as a combination of the previous processes. It contains one function `A_dep_r()`, which includes one parameter `S_f`, defining the function of the site. If `S_f = NULL`, then the site function has no effect. Alternatively, the parameter `Pop` can be introduced manually, or retrieved from the previuos protocol. If the parameter `Pop = NULL`, population has no effect on deposition rates. If both `Pop` and `S_f` are active, they both configure the final `Adr` outcome. However, if both are `NULL`, then the function will throw an error, and the user will have to introduce manually the anthropogenic deposition rates for the computation of $W$ in subsequent processes.
...
The user can establish deposition rates with a fixed number or high or low.
