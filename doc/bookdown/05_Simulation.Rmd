...
The dynamics of the simulations will heaivly depend on the initial condition and the parameters. As an exemple, wiht these paraemeters, it is not rare than the Farmers, due to the implementation of the conlfict described in the Chapter&nbsp;\@ref(conflict), will destroy themsleve by going to war too often. This can be avoid with the current implementation by setting a small probfight, to 0.1; but it is important to remember that it could have been done via various way. A more clever one would have been for Farmers to 'estimates' the likelihood for them to loose the battle, maybe buy fighting only _safe_  battles, against way smaller settelements.

In order to find the right simulation, that will show the dynamics you want to challenge your participants to detect, you will want to play with these parameters and run a couple of replication of simulation in order to find the right one. 
...
```{r multi-run,out.width="33%",fig.cap="running multiple simulation",fig.show="hold"}
...
for(expname in paste0("exp",1:10)){
...
```{r explore-mult,out.width="33%",fig.cap="Comparing multiple runs",fig.show="hold"}
    ...
    plot(1,i,xlim=c(0,10),ylim=c(0,gnmax))
    ...
    #plot(1:i,warcasualties[1:(i-1)],lwd=2,col="green",type="h",yaxt="n",ylab="",xlim=c(0,i))
    #axis(4)
    #par(new=T)
