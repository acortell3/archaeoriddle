...
initKs <- function(Kbase=c("HG"=30,"F"=120),sites,ressources){
    Ks=Kbase[sites$culture]+round(rnorm(length(sites),0,10))
...
sites=vect(readRDS("goodiste_init.RDS"))
INs=round(runif(length(sites),.5,1.5)*sites$Ks) #Population size at initialisation
...
```{r,eval=F}
plot(sites,cex=(1+Nts[1,]/100),pch=21,add=T,bg=rainbow(2,alpha=.6)[as.factor(sites$culture)])
```
...
for (i in 2:(ts+1)){
    ...
    for(s in sample(seq_along(sites))){
        ...
        if(newN>=(Ks[[s]])){ #if new population is more than carrying capacity: migration scenario
            ...
            new_site=NULL
            #print(paste("possib",migrants))
            if(migrants>=.2*Ks[[s]] ){ #if supropulation > 10 people, 10% chance of creation of a new city

                print(paste("look for new spot",migrants))
                tmp=Nts[i-1,]
                tmp[Nts[i,]>0]=Nts[i,Nts[i,]>0]
                territory=erase(viable,buffer(sites,(tmp)/2*1000))
            ...
            if(length(new_site)==0 && runif(1)>.1 ){
                ...
                attractivity=attractivity*(1+(sites$culture[s]==sites$culture)) #2 times more likely to go to similar culture
