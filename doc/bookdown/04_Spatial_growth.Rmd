...
#but we got those saved in a RDS so:
            ...
            if(migrants>= (minimals[sites$culture[s]]*sites$Ks[s]) & runif(1)<prob_split[sites$culture[s]] ){ #if supropulation > 10 people, 10% chance of creation of a new city
                ...
                if(length(territory)>0){
                    ...
                    if(length(new_site)>0 & all(!is.na(crds(new_site)))){
                        ...
                        new_site$Ks=round(initKs(Kbase,sites=new_site,ress,sizeex="F"))
...
#print(paste("from",sites$culture[s],"to",sites$culture[city]))
    ...
    for(s in sample(which(sites$culture=="F" & Nts[i,]>0))){
        ...
        if(length(clash)>0 && !is.na(clash) && runif(1)<.1){
            ...
            Nts[i,]=newns
            print(paste0("results :", s," (",cultures[s],") with ",Nts[i,s]," individual, attacked: ",attack," (",cultures[attack],") with ",Nts[i,attack]," individual, #casualties=",casualties))
        }
    }
    if(visumin)dev.off()
}
...
```{r}
plot(apply(Nts[1:i,sites$culture=="F"],1,sum),col="red",type="l",lwd=2)
...
par(new=T)
plot(warcasualties[1:(i-1)],lwd=2,col="green",type="l",yaxt="n",ylab="")
axis(4)
```
