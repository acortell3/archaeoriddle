...
if(visu){
    ...
    for(s in sample(which(sites$culture=="F" & Nts[i,]>0))){
        ...
        if(length(clash)>0){
            ...
            sizew=sum(Nts[i,c(s,attack)]-newns[c(s,attack)])^2/1000
            warpoints(sites,s,attack,Ne=Nts[i,],buffersize=buff,sizewar=)
            Nts[i,]=newns
    #effectively kill people in population (could be done taking into account the age to be more realistic)
            Ips[[s]]=reducePop(Ips[[s]],Nts[i,s])
            Ips[[attack]]=reducePop(Ips[[attack]],Nts[i,attack])
        }
    }
    if(visumin)dev.off()
}
