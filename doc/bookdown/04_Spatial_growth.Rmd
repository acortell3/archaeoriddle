...
ts=20000
Kbase=c("HG"=35,"F"=150) #difference in K for the two cultures
minimals=c("HG"=.14,"F"=.21) #how big the group of migrant should be to create a new city vs migrate to a existing one 
...
prob_survive=c("HG"=0.8,"F"=0.5)
prob_split=c("HG"=.1,"F"=.7)
prob_move=c("HG"=.2,"F"=.2)
...
plot(sites,cex=(.3+Nts[1,]/200),pch=21,add=T,bg=rainbow(2,alpha=.6)[as.factor(cultures)])
plot(ress,add=T,alpha=.2,col=colorRampPalette(c("white","yellow","orange","red"))(80))
tmp=Nts[1,]
buffersize=(sqrt(tmp)+penal_cul[cultures])*3000*runif(1,.5,1.5)
territory=erase(viable,buffer(sites,buffersize))
plot(territory,add=T,alpha=.2,col="red")
###
    ...
    if(visu){
        ...
        if(newN>=(Ks[[s]])){ #if new population is more than carrying capacity: migration scenario
            ...
            if(migrants>= (minimals[sites$culture[s]]*sites$Ks[s]) & runif(1)<prob_split[sites$culture[s]] ){ #if supropulation > 10 people, 10% chance of creation of a new city
                ...
                buffersize=(sqrt(tmp)+penal_cul[cultures])*2000*runif(1,.5,1.5)
                buffersize[tmp==0]=0
                territory=erase(viable,buffer(sites,buffersize))
                ...
                if(length(territory)>0){
                    ...
                    if(length(new_site)>0 & all(!is.na(crds(new_site)))){
                        ...
                        print(paste("new settlement (",sites$culture[s],")"))
            ...
            if(length(new_site)==0 && runif(1)<prob_move[sites$culture[s]] ){
                ...
                attractivity=attractivity*(1+5*(sites$culture[s]==sites$culture)) #4 times more likely to go to similar culture
                attractivity[s]=min(attractivity)-1
                attractivity=exp(attractivity)/sum(exp(attractivity))
                attractivity[Nts[i-1,]==0]=0 
                city=sample(size=1,x=seq_along(sites),prob=attractivity)
                Nts[i,city]=Nts[i-1,city]+migrants
#print(paste("from",sites$culture[s],"to",sites$culture[city]))
                havemoved=T
            }
            if(havemoved){
...
if(visu){
    ...
    for(s in sample(which(sites$culture=="F" & Nts[i,]>0))){
        ...
        if(length(clash)>0 && !is.na(clash) && runif(1)<.1){
            ...
            newns=simplefight(Ne=Nts[i,],a=s,b=attack)
            sizew=sum(Nts[i,c(s,attack)]-newns[c(s,attack)])^2/4000
            warpoints(sites,s,attack,Ne=Nts[i,],buffersize=buff,sizewar=sizew+.5)
            
            print(paste(s,"(",cultures[s],") attack:",attack,"(",cultures[attack],")","war size",sqrt(sizew)*4000))
...
```{r,fig.width="25%"}
4lldeposit=lapply(1:ncol(Nts),function(i)Rec_c(sapply(Nts[,i],A_rates), InitBP = 15000,ts=ts,r = 0.2, Max_bone_thickness = "m"))
...
lapply(1:ncol(Nts),function(i)lines(Nts[,i],col=rainbow(2)[as.factor(cultures)[i]]))
...
```{r}
plot(apply(Nts[1:2865,sites$culture=="HG"],1,sum),col="red",type="l",lwd=2)
 points(apply(Nts[1:2865,sites$culture=="F"],1,sum),col="blue",lwd=2,type="l")
```
