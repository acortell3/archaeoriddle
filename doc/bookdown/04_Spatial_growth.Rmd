...
onesimu=run_simulation (
                           sites=vect(readRDS("testsites.RDS")),
                           dem=rast("east_narnia4x.tif"),
                           ressources=ress,
                           water=height.wat,
                           foldervid=paste0("pathtofinal_",i),
                           visu=F,visumin=TRUE,
                           ts=120,#length of simulation in year
                           Kbase=c("HG"=35,"F"=120),#difference in K for the two cultures
                           cul_ext=c("HG"=7,"F"=6),#spatial penality to extent: lower, bigger penality
                           penal_cul=c("HG"=4,"F"=5),#penality of occupational area: low, other sites can cam close
                           prob_birth=c("HG"=0.3,"F"=0.5),#proba to give birth every year
...
```{r,cache=F}
onesimu=readRDS("buffattack300_K110_PSU065_5_all.RDS")
Nts=onesimu$Nts # population at each timestep
warcasualties=onesimu$warcasualties #death by war at each time step
sites_new=terra::vect(readRDS("buffattack300_K110_PSU065_5_sitesRast.RDS"))
```


Use ressource to adjust the Ks:
```{r,eval=T}
plot(sites_new,cex=(1+Nts[1,]/100),pch=21,bg=rainbow(2,alpha=.6)[as.factor(sites_new$culture)])
```
...
lapply(1:ncol(Nts),function(j)lines(Nts[,j],col=rainbow(2)[as.factor(sites_new$culture)[j]]))
