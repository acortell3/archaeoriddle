...
K=100
INs=round(runif(length(ss),.5,1.5)*100) #Population size at initialisation
INs=round(runif(length(sites),.5,1.5)*100) #Population size at initialisation
...
plot(sitesrcex=(1+Nts[1,]/100),pch=21,add=T,bg=rainbow(n,alpha=.6))
for (i in 2:(ts+1)){
    ...
    for(s in sample(seq_along(sites))){
        ...
        if(newN>(Ks[[s]])){ #migration scenario
            ...
            if(migrants>20){
                territory=erase(viable,buffer(sites,10000))
                new=spatSample(territory,1)
                sites=rbind(sites,new)
        ...
        if(!is.null(city)){
            ...
            #plot(height.ras^1.9,col=col_ramp(50),legend=F,reset=F)
            #plot(height.wat,col="lightblue",add=T,legend=F)
            #plot(sites,cex=(1+Nts[1,]/100),pch=21,add=T,bg=rainbow(n,alpha=.6),lwd=siteslwd,col=sitescols)
            #plot(st_cast(st_combine(st_as_sf(sites[c(s,city)])),"LINESTRING"),add=T)
        }
    }
}
```
...
```{r}
plot(1,1,type="n",xlim=c(0,max(ts)),ylim=c(0,max(Nts)))
apply(Nts,2,lines)
territory=st_union(st_as_sf(buffer(sites,5000)))
inter<Plug>Tex <- MathCalk 
