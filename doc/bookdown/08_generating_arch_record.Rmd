...
Here we will use all these previous steps to generate csvs that can be shared. In this chapter we will use the simulation run in the Chapter&nbsp;\@ref(main-simu) ; if you want to generate new 'fake'-dataset, different from the one that has been presented in the original challenge and that will be described in the chapter&nbsp;\@ref(original-challenge), you will want to run longer simulation, and choose among different runs the ones that look interesting/realistic. We provide a script `./scriptmini.R` to give some idea about how to automatize these runs and how to choose among them.
...
Once this is done, we have a list with all the potential dates available from all sites that have existed during our simulation. Realistically, for any given site, not _all_  the material will be recovered and even less of this material will be dated. To reflect that, we capped the maximum number of samples available per sites. To do so -- and this was applied as is in the original challenge presented in the Chapter&nbsp;\@ref(original-challenge), we assume that the biggest sites -- the ones with most media attention, most funding, will very likely get at most 30 dates. We thus subsampled the number of dates for smaller sites given this ratio of $\frac{30}{n_{\text{largest}}}$. Here the simulation has been greatly shortened, thus sites have way less samples. We thus cap the large sites to have only 80% of the dates they generate, and sample from the other sites using this ratio.
...
To then share these date we choose to split the environment in grids and extract dates by grid. For each grid, a csv is generated with the sites that have been found and for each site, their geolocalisation and the dates found at the site.
...
In the archaoriddle original challenge we proposed to give the data from 4 grids that we selected ourselves to be sure the participants have a least _some_  interesting information. Here we randomly pick up 4 squares with the only limits that they provide at least 5 settlements. You will want to change and manually the vector `selection` in the chunk below to cover areas with lot of sites and sites from different cultures, from what can be seen in the Figure&nbsp;\@ref(fig:plotallfoundiste-ex).
...
On the original challenge we used: [fantasynamegenerators.com](https://www.fantasynamegenerators.com/fantasy-town-names.php) to choose nice, randomly generated names, to name the sites publicly available. You will see in the section&nbsp;\@ref(naming) that we positioned them with culturally relevant groups, although this has no impact on the results. Here we took all the names we had for the original challenge to randomly assign some for you custom world.
