...
```{r}
## See some of the archaeoriddle functions
```
```{r, echo=FALSE}
help.search(
...
In order to account for the chronological occupation of the site, a demographic process must be produced, where the sample is generated accordingly. This is described in the previous chapter, but we can use the function `Pop_stoch`  to generate a simple stochastic growth and apply our loss function to it.
